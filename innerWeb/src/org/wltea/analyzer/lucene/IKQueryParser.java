...
public final class IKQueryParser {
	 ...
	 * 真对搜索页面，一个输入框，一个单选按钮
	 * 一个keyword对应一个fields
	 * 另外一个keyword对应一个field
	 * @author ZY
	 * @date 2010-11-6 下午08:55:26
	 * @param fields
	 * @param keyword1
	 * @param field
	 * @param keyword2  TODO 如何解决分类ID虽然作为关键字，但是高亮的时候，把这个关键词给剔除掉
	 * @return
	 * @throws IOException
	 */
	public static Query parseMultiField(String[] fields,String keyword1,String field,String keyword2) throws IOException{
		BooleanQuery resultQuery = new BooleanQuery();
		Query partQuery1 = parseMultiField(fields, keyword1);
		if(partQuery1 != null && 
		          (!(partQuery1 instanceof BooleanQuery) || ((BooleanQuery)partQuery1).getClauses().length>0)){
			resultQuery.add(partQuery1, Occur.MUST); 
		}
			
		Query partQuery2 = _parse(field,keyword2);
		if(partQuery2 != null && 
		          (!(partQuery2 instanceof BooleanQuery) || ((BooleanQuery)partQuery2).getClauses().length>0)){
			resultQuery.add(partQuery2, Occur.MUST); 
		}
		
		/*Query partQuery2 = NumericRangeQuery.newFloatRange(field, //field name  
...
*/			
	...
	/**
	 * TODO 高亮显示是否能指定关键字，文档中没有找到，暂时用了一个超级笨的方法。
	 * 可以使用TermRangeFilter进行过滤查询，就是指定在某个字段的范围内进行查询。例如：数字，日期等 还有一个NumericRangeQuery 但是API指明，这个方法目前正在测试阶段
	 * 替换切词后的高亮显示部分
	 * @author ZY
	 * @date 2010-11-6 下午11:20:28
	 * @param queryWord
	 * @param content
	 * @param preTag
	 * @param postTag
	 * @return
	 */
	public static String getHighlightString(String queryWord,String content,String preTag,String postTag){
		List<String> list = new ArrayList<String>();
		String result = content ;
		if(StringUtils.isBlank(queryWord) || StringUtils.isBlank(content))
			return "" ;
		IKTokenizer tokenizer = new IKTokenizer(new StringReader(queryWord) , false);
		try {
			while(tokenizer.incrementToken()){ // 这个是按照最小力度来切分词，从最小颗粒，然后递增
			...
			for(String t : ll){
				result = result.replaceAll(t, preTag.concat(t).concat(postTag));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
