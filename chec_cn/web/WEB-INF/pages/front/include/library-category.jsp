...
<SCRIPT type="text/javascript">
    ...
    var zNodes = [
        ...
        { id: "${lib.id}", pId: "${lib.parentId}", name: "${fmtString:substringAppend(lib.name,20,'..')}", open: false, url: "${basePath}${lib.accessUrl}",target:"_self",icon:"${basePath}res/front/library/images/fenlei_con1.jpg"}<c:if test="${!vStatus.last}">,</c:if>
        </c:when>
        <c:otherwise>
        { id: "${lib.id}", pId: "${lib.parentId}", name: "${fmtString:substringAppend(lib.name,20,'..')}", url: "${basePath}${lib.accessUrl}",target:"_self", icon:"${basePath}res/front/library/images/notice_list.png"}<c:if test="${!vStatus.last}">,</c:if>
        </c:otherwise>
        </c:choose>
        </c:forEach>
    ];
