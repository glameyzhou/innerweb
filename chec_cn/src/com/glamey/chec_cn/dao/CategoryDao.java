...
public class CategoryDao extends BaseDao {
        ...
        try {
            int count = jdbcTemplate.update(
                    "insert into tbl_category" +
                            "(id,name,shortname,aliasname,categorydescribe,showtype,showindex,showintree,treeorder,categoryorder,parentid,categorytype,categoryimage,categoryflash,categorytime) " +
                            "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,now())",
                    new PreparedStatementSetter() {
                        ...
                        public void setValues(PreparedStatement pstmt) throws SQLException {
                            ...
                            pstmt.setString(++i, category.getCategoryFlash());
                        }
                    });
            return count > 0;
        } catch (Exception e) {
    ...
    public String createReturnId(final Category category) {
        logger.info("[CategoryDao] #createReturnId# " + category);
        try {
            final String id = StringTools.getUniqueId();
            int count = jdbcTemplate.update(
                    "insert into tbl_category" +
                            "(id,name,shortname,aliasname,categorydescribe,showtype,showindex,showintree,treeorder,categoryorder,parentid,categorytype,categoryimage,categoryflash,categorytime,hasChild) " +
                            "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,now(),?)",
                    new PreparedStatementSetter() {
                        @Override
                        public void setValues(PreparedStatement pstmt) throws SQLException {
            ...
            logger.error("[CategoryDao] #createReturnId# error " + category, e);
            return null;
        }
    }
    ...
    public boolean update(final Category category) {
        ...
        try {
            int count = jdbcTemplate.update(
                    "update tbl_category set name=?,shortname=?,aliasname=?,categorydescribe=?,showtype=?,showindex=?,showintree=?,treeorder=?,categoryorder=?,parentid=?,categorytype=? ,categoryimage = ?, categoryflash = ?  where id = ?",
                    new PreparedStatementSetter() {
                        @Override
                        public void setValues(PreparedStatement pstmt) throws SQLException {
                            int i = 0;
            ...
            logger.error("[CategoryDao] #update# error " + category, e);
            return false;
        }
    }
    ...
    class CategoryRowMapper implements RowMapper<Category> {
        ...
        public Category mapRow(ResultSet rs, int i) throws SQLException {
            ...
            category.setCategoryFlash(rs.getString("categoryflash"));
            category.setCategoryTime(rs.getTimestamp("categorytime"));
